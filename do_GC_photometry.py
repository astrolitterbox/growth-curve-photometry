...
for i in range(1, 940):
	...
	skyM = db.dbUtils.getFromDB('sky', dbDir+'CALIFA.sqlite', 'sky_new', ' where califa_id = '+str(i))[0]	
	print sky, skyM
	sky = 108.6
	...
	NpixM = data[:, 4] #current flux
	print currentFlux[10], Npix[10]
	currentFlux = np.subtract(currentFlux, Npix*sky)
	#currentFlux = sky
	print currentFlux[10]	
