...
class Photometry():
  ...
  @staticmethod
  def getStart(CALIFA_ID):
    out = np.genfromtxt(Photometry.getOutputFilename(CALIFA_ID)).shape[0]
  ...
  @staticmethod
  def getOutputFilename(CALIFA_ID):
    return 'growth_curves/'+Settings.getConstants().band+'/gc_profile'+CALIFA_ID+'.csv'
